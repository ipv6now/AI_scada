...
from PyQt5.QtWidgets import (
    ...
    QGraphicsPixmapItem, QApplication, QDialogButtonBox
)
...
class VariableBinding:
    ...
    def __init__(self, variable_name='', variable_type='read', address='', description=''):
...
class HMIObject:
    ...
    def add_variable_binding(self, variable_name, variable_type, address, description=''):
        var = VariableBinding(variable_name, variable_type, address, description)
...
class HMITrendChart(HMIObject):
    ...
    def draw(self, scene):
        ...
        font = QFont()
        font.setFamily(self.properties.get('font_family', 'Microsoft YaHei'))
        font.setPointSize(self.properties.get('font_size', 14))
        text.setFont(font)
        text.setDefaultTextColor(QColor(self.properties.get('color', '#000000')))
...
class HMIDesigner(QDialog):
    ...
    def create_right_panel(self):
        ...
        """)
        btn_select.clicked.connect(lambda: self.set_tool("select"))
    ...
    def create_properties_panel(self):
        ...
        # Available variables from data_manager
        avail_var_group = QGroupBox("可用变量")
        avail_var_layout = QVBoxLayout()
        avail_var_layout.setSpacing(5)
        avail_var_layout.setContentsMargins(5, 5, 5, 5)
        
        self.avail_var_combo = QComboBox()
        self.avail_var_combo.setMinimumWidth(150)
        self.refresh_available_variables()
        avail_var_layout.addWidget(QLabel("选择变量:"))
        avail_var_layout.addWidget(self.avail_var_combo)
        
        self.refresh_var_btn = QPushButton("刷新变量列表")
        self.refresh_var_btn.clicked.connect(self.refresh_available_variables)
        avail_var_layout.addWidget(self.refresh_var_btn)
        
        avail_var_layout.addStretch()
        avail_var_group.setLayout(avail_var_layout)
        self.var_splitter.addWidget(avail_var_group)
        
        # Variable list (bound variables)
        var_table_group = QGroupBox("已绑定变量")
        var_table_layout = QVBoxLayout()
        var_table_layout.setSpacing(5)
        var_table_layout.setContentsMargins(5, 5, 5, 5)
        
        self.var_table = QTableWidget()
        self.var_table.setColumnCount(4)
        self.var_table.setHorizontalHeaderLabels(["变量名", "类型", "地址", "读写"])
        self.var_table.horizontalHeader().setSectionResizeMode(QHeaderView.Stretch)
        var_table_layout.addWidget(self.var_table)
        
        var_table_group.setLayout(var_table_layout)
        self.var_splitter.addWidget(var_table_group)
        
        # Variable controls
        var_ctrl_group = QGroupBox("操作")
        var_ctrl_layout = QVBoxLayout()
        var_ctrl_layout.setSpacing(5)
        var_ctrl_layout.setContentsMargins(5, 5, 5, 5)
        
        self.add_var_btn = QPushButton("添加绑定")
        self.add_var_btn.clicked.connect(self.add_variable_binding_from_combo)
        var_ctrl_layout.addWidget(self.add_var_btn)
        
        self.remove_var_btn = QPushButton("移除绑定")
        self.remove_var_btn.clicked.connect(self.remove_variable_binding)
        var_ctrl_layout.addWidget(self.remove_var_btn)
        
        var_ctrl_layout.addStretch()
        var_ctrl_group.setLayout(var_ctrl_layout)
        self.var_splitter.addWidget(var_ctrl_group)
        
        var_main_layout.addWidget(self.var_splitter)
        var_tab.setLayout(var_main_layout)
        self.props_tab_widget.addTab(var_tab, "变量绑定")
        
        # Tab 3: Advanced Properties - With resizable splitter layout
        adv_tab = QWidget()
    ...
    def _create_control_specific_properties_with_splitter(self):
        ...
        var_op_layout.setSpacing(5)
        var_op_layout.setContentsMargins(5, 5, 5, 5)
        ...
        var_op_layout.addWidget(QLabel("变量:"))
        self.target_var_combo = QComboBox()
        self.target_var_combo.setEditable(True)
        self.target_var_combo.setFixedWidth(90)
        self.target_var_combo.currentTextChanged.connect(self.on_property_change)
        var_op_layout.addWidget(self.target_var_combo)
        
        refresh_var_btn = QPushButton("⟳")
        refresh_var_btn.setFixedSize(20, 20)
        refresh_var_btn.setToolTip("刷新变量")
        refresh_var_btn.clicked.connect(self.refresh_target_variables)
        var_op_layout.addWidget(refresh_var_btn)
    ...
    def update_properties_panel(self):
        ...
        if len(self.selected_objects) > 1:
            ...
            # Update variable table - might show aggregate info
            ...
            if obj.obj_type in ['button', 'switch']:
                ...
                if action_type == '画面跳转':
                    ...
                    # Variable operation
                    action_text = var_operation if var_operation else '置位'
                
                idx = self.var_operation_combo.findText(action_text)
                if idx >= 0:
                ...
                self.on_action_type_change(action_text)
                
                # Refresh and set target variable
                self.refresh_target_variables()
                self.target_var_combo.blockSignals(True)
                target_var = props.get('target_variable', '')
                if target_var:
                    idx = self.target_var_combo.findText(target_var)
                    if idx >= 0:
                        self.target_var_combo.setCurrentIndex(idx)
                    else:
                        self.target_var_combo.setEditText(target_var)
                self.target_var_combo.blockSignals(False)
        ...
        if self.selected_objects:
            ...
            for obj in self.selected_objects:
                ...
                if obj.obj_type == 'button':
                    ...
                    action_text = self.var_operation_combo.currentText()
                    if action_text == "画面跳转":
                        ...
                        obj.properties['variable_operation'] = action_text
    ...
    def refresh_target_variables(self):
        ...
        if self.data_manager and hasattr(self.data_manager, 'tags'):
            for tag_name in sorted(self.data_manager.tags.keys()):
                self.target_var_combo.addItem(tag_name)
        self.target_var_combo.blockSignals(False)
    ...
    def refresh_available_variables(self):
        ...
        if self.data_manager and hasattr(self.data_manager, 'tags'):
            for tag_name in sorted(self.data_manager.tags.keys()):
                ...
                display_text = f"{tag_name} ({tag.address})"
                self.avail_var_combo.addItem(display_text, tag_name)
    
    def add_variable_binding_from_combo(self):
        """Add a variable binding from the combo box selection"""
        if self.selected_object and self.avail_var_combo.currentData():
            tag_name = self.avail_var_combo.currentData()
    ...
    def add_variable_binding(self):
        """Add a variable binding to the selected object"""
        if self.selected_object:
            # For simplicity, add a placeholder variable
            self.selected_object.add_variable_binding("NewVariable", "read", "", "")
            self.update_variable_table()
    
    def create_general_properties(self):
        """Create general properties content for the property tab"""
        # Clear existing layout
        for i in reversed(range(self.prop_content_layout.count())):
            widget = self.prop_content_layout.takeAt(i).widget()
            if widget:
                widget.deleteLater()
        
        # General properties group
        general_group = QGroupBox("常规属性")
        ...
        general_layout.addLayout(color_layout)
        ...
        general_layout.addLayout(border_layout)
        
        general_group.setLayout(general_layout)
        self.prop_content_layout.addWidget(general_group)
        
        # Button/Switch specific - Action type
        self.action_group = QGroupBox("控件动作")
        ...
        action_type_layout.addWidget(QLabel("动作:"))
        self.action_combo = QComboBox()
        self.action_combo.addItems(["无", "变量操作", "画面跳转"])
        self.action_combo.currentTextChanged.connect(self.on_action_type_change)
        ...
        # Variable operation settings (compact horizontal layout)
        self.var_op_group = QGroupBox()
        self.var_op_group.setStyleSheet("QGroupBox { border: none; padding: 0px; margin: 0px; }")
        var_op_layout = QHBoxLayout()
        var_op_layout.setSpacing(5)
        var_op_layout.setContentsMargins(0, 0, 0, 0)
        
        # Operation type
        var_op_layout.addWidget(QLabel("操作:"))
        self.var_operation_combo = QComboBox()
        self.var_operation_combo.addItems(["置位", "复位", "取反", "置1", "置0", "加1", "减1", "点动"])
        self.var_operation_combo.currentTextChanged.connect(self.on_property_change)
        self.var_operation_combo.setFixedWidth(70)
        var_op_layout.addWidget(self.var_operation_combo)
        ...
        self.target_var_combo = QComboBox()
        self.target_var_combo.setEditable(True)
        self.target_var_combo.setFixedWidth(120)
        self.target_var_combo.currentTextChanged.connect(self.on_property_change)
        var_op_layout.addWidget(self.target_var_combo)
        
        # Refresh button
        refresh_var_btn = QPushButton("⟳")
        refresh_var_btn.setFixedSize(24, 24)
        refresh_var_btn.setToolTip("刷新变量")
        refresh_var_btn.clicked.connect(self.refresh_target_variables)
        var_op_layout.addWidget(refresh_var_btn)
        
        self.var_op_group.setLayout(var_op_layout)
        action_layout.addWidget(self.var_op_group)
        
        # Screen navigation settings (compact horizontal layout)
        self.screen_nav_group = QGroupBox()
        self.screen_nav_group.setStyleSheet("QGroupBox { border: none; padding: 0px; margin: 0px; }")
        screen_nav_layout = QHBoxLayout()
        screen_nav_layout.setSpacing(5)
        screen_nav_layout.setContentsMargins(0, 0, 0, 0)
        
        # Target screen selection
        screen_nav_layout.addWidget(QLabel("画面:"))
        self.target_screen_combo = QComboBox()
        self.target_screen_combo.setEditable(True)
        self.target_screen_combo.setFixedWidth(100)
        self.target_screen_combo.currentTextChanged.connect(self.on_property_change)
        screen_nav_layout.addWidget(self.target_screen_combo)
        
        # Target screen number
        screen_nav_layout.addWidget(QLabel("编号:"))
        self.target_screen_number_spin = QSpinBox()
        self.target_screen_number_spin.setRange(0, 999)
        self.target_screen_number_spin.setFixedWidth(50)
        self.target_screen_number_spin.valueChanged.connect(self.on_property_change)
        screen_nav_layout.addWidget(self.target_screen_number_spin)
        
        # Refresh button
        refresh_screen_btn = QPushButton("⟳")
        refresh_screen_btn.setFixedSize(24, 24)
        refresh_screen_btn.setToolTip("刷新画面")
        refresh_screen_btn.clicked.connect(self.refresh_target_screens)
        screen_nav_layout.addWidget(refresh_screen_btn)
        
        self.screen_nav_group.setLayout(screen_nav_layout)
        action_layout.addWidget(self.screen_nav_group)
        
        # Toggle mode (for switch)
        self.toggle_checkbox = QCheckBox("切换模式")
        self.toggle_checkbox.stateChanged.connect(self.on_property_change)
        action_layout.addWidget(self.toggle_checkbox)
        
        self.action_group.setLayout(action_layout)
        self.prop_content_layout.addWidget(self.action_group)
        
        self.prop_content_layout.addStretch()

    def on_property_category_changed(self, item, column):
        """Handle property category selection changes"""
        ...
        # Create content based on selected category
        if category == "常规":
            self.create_general_properties()
        elif category == "填充":
            # Create fill properties
            fill_group = QGroupBox("填充属性")
            ...
            self.filled_checkbox = QCheckBox("填充")
            self.filled_checkbox.stateChanged.connect(self.on_property_change)
            filled_layout.addWidget(self.filled_checkbox)
            filled_layout.addStretch()
            fill_layout.addLayout(filled_layout)
            
            # Fill color
            fill_color_layout = QHBoxLayout()
            fill_color_layout.addWidget(QLabel("填充颜色:"))
            self.fill_color_button = QPushButton("选择")
            ...
            self.prop_content_layout.addWidget(fill_group)
        elif category == "外观样式":
            # Create appearance properties
            appear_group = QGroupBox("外观样式")
            ...
            appear_layout.addLayout(line_width_layout)
            
            appear_group.setLayout(appear_layout)
            self.prop_content_layout.addWidget(appear_group)
        
        self.prop_content_layout.addStretch()

    def remove_variable_binding(self):
        """Remove a variable binding from the selected object"""
        if self.selected_object:
            current_row = self.var_table.currentRow()
            if current_row >= 0:
                self.selected_object.remove_variable_binding(current_row)
                self.update_variable_table()
    ...
    def _save_screen_to_file(self):
        ...
        try:
            ...
            for screen in self.screens:
                ...
                project_data['hmi_screens'].append(screen_data)
    ...
    def load_screen(self):
        ...
        try:
            ...
            # Load screens
            for idx, screen_data in enumerate(project_data.get('hmi_screens', []) if 'hmi_screens' in project_data else project_data.get('screens', [])):
